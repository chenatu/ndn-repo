...
public:
	...
	virtual int check_name_any(const Name& name, const Selectors& selectors, vector<Name>& vname);
...
//private:
	...
	int delete_data(const Name& name);
	...
	int check_data_name(const Name& name, vector<Name>& vname);
	//MinSuffixComponents filter
	int check_name_minsuffix(const Name& name, int minSuffixComponents, vector<Name>& vname);
	//MaxSuffixComponents filter
	int check_name_maxsuffix(const Name& name, int maxSuffixComponents, vector<Name>& vname);
	//Exclude filter
	int check_name_exclude(const Name& name, const Exclude& exclude, vector<Name>& vname);
	//ChildSelector filter
	int check_name_child(const Name& name, int childselector, vector<Name>& vname, Name& resname);
